...
export function InvoiceForm({ invoice, suppliers, projects, purchaseOrders, onSave, onCancel }: InvoiceFormProps) {
  ...
  const selectedPOId = useWatch({ control: form.control, name: 'purchaseOrderId' });
  ...
  const selectedOrderForPreview = useMemo(() => {
    if (!selectedPOId) return null;
    return purchaseOrders.find(po => po.id === selectedPOId);
  }, [selectedPOId, purchaseOrders]);

  const selectedProjectForPreview = useMemo(() => {
    if (!selectedOrderForPreview) return null;
    return projects.find(p => p.id === selectedOrderForPreview.project);
  }, [selectedOrderForPreview, projects]);
